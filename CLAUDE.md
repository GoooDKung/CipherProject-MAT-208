...
| Live-demo GUI (web) | Streamlit app — same math/cases as the desktop GUI (imports `cipher_tools` and `visualize.CASES`/`draw_*` directly, no duplicated logic), runs in any browser so it opens on any device without a Python install for viewers if deployed to Streamlit Community Cloud. Theme-neutral CSS only (no hardcoded backgrounds) so it's legible in both Streamlit light and dark mode. Sidebar: message input, K-matrix number inputs, 4 preset buttons (`Preset 1: Invertible (det=9)` … `Preset 4: Factor 13 (det=13)`). Main panel: det/gcd/invertibility metric cards, full digraph-by-digraph encoding table, and a Recovery section — invertible case shows det⁻¹, K⁻¹, a decoded-pipeline table (ciphertext → K⁻¹w mod 26 → recovered letters), and the recovered plaintext with padding called out; singular case shows a SingularKeyError alert, kernel cardinality/image contraction, and an explicit collision proof box (v1, v2 = v1+v0, both ciphertext images) — plus the live scatter/heatmap plot. Run: `streamlit run src/app.py` | `src/app.py` |
